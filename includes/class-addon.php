...
if( ! class_exists( 'Inventory_Presser_Addon' ) )
{
	...
	{		
		...
		public static function add_license_key_box( $option_name, $settings_section, $sanitize_values_hook_name )
		{
			add_action( 'admin_init', function() use ( $option_name, $settings_section ) {
				...
				$current_value = empty( $option['license_key'] ) ? '' : $option['license_key'];

				add_settings_field(
					'license_key', // id
					__( 'License key', 'inventory-presser' ), // title
					function() use ( $option_name, $current_value ) {
