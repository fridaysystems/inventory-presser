...
class Fuel_Economy_Widget extends WP_Widget {
	 ...
	 */
 	public function widget( $args, $instance ) {
		...
		foreach( $epa['Fuels'] as $fuel ) {
			...
			if( isset( $fuel['Attributes'] ) && isset( $fuel['Attributes']['Combined MPG'] ) ) {
				$have_combined = true;
				printf( '<span class="number">%s</span>', $fuel['Attributes']['Combined MPG'] );
			}
			...
			if( isset( $fuel['Attributes'] ) && isset( $fuel['Attributes']['City MPG'] ) ) {
				$have_city = true;
				printf( '<span class="number">%s</span>', $fuel['Attributes']['City MPG'] );
			}
			echo '</td><td class="fuel-economy-highway">';
			if( isset( $fuel['Attributes'] ) && isset( $fuel['Attributes']['Highway MPG'] ) ) {
				$have_highway = true;
				printf( '<span class="number">%s</span>', $fuel['Attributes']['Highway MPG'] );
			}
			printf(
 	...
 	function mpg_number_html( $number, $context ) {
 		return sprintf(
 			'<div class="fuel-economy-number fuel-economy-%s">'
 			. '<span class="number">%s</span>'
 			. '<span class="context">%s</span></div>',
 			strtolower( $context ),
			$number,
			$context
		);
