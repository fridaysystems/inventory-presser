...
export default function Edit( { isSelected } ) {
	...
	if ( ! isSelected ) {
		...
		return ( <div { ...blockProps }> { meta[ invp_blocks.meta_prefix + 'beam' ] } </div> );
	}
	...
	return (
		<>
			<TextControl
				label    = { 'Beam' }
				value    = { meta[ invp_blocks.meta_prefix + 'beam' ] }
				onChange = {(newValue) => setMeta( { ...meta, [invp_blocks.meta_prefix + 'beam']: newValue } )}
			/>
		</>
	);
}
