...
export default function Edit( { isSelected, context } ) {
	...
	if ( ! isSelected ) {
		...
		return ( <div { ...blockProps }> { invpFormatCurrency( meta[ invp_blocks.meta_prefix + 'price' ] ) } </div> );
	}
	...
	return (
		<div { ...blockProps }>
			<TextControl
				...
				onChange = {(newValue) => setMeta( { ...meta, [invp_blocks.meta_prefix + 'price']: newValue.replace( /[^0-9\.]/g, '' ) } )}
			/>
		</div>
	);
}
