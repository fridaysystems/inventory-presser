...
export default function Edit( { isSelected, context } ) {
	...
	if ( ! isSelected ) {
		...
		return ( <div { ...blockProps }> { meta[ invp_blocks.meta_prefix + 'color' ] } </div> );
	}
	...
	return (
		<div { ...blockProps }>
			<TextControl
				...
				value    = { meta[ invp_blocks.meta_prefix + 'color' ] }
				onChange = {(newValue) => setMeta( { ...meta, [invp_blocks.meta_prefix + 'color']: newValue } )}
			/>
		</div>
	);
}
