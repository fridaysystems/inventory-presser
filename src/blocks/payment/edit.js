...
export default function Edit( { isSelected, context } ) {
	...
	if ( ! isSelected ) {
		...
		return ( <div { ...blockProps }> { invpFormatCurrency( meta[ invp_blocks.meta_prefix + 'payment' ] ) } </div> );
	}
	...
	return (
		<div { ...blockProps }>
			<TextControl
				...
				value    = { meta[ invp_blocks.meta_prefix + 'payment' ] }
				onChange = {(newValue) => setMeta( { ...meta, [invp_blocks.meta_prefix + 'payment']: newValue.replace( /[^0-9\.]/g, '' ) } )}
			/>
		</div>
	);
}
