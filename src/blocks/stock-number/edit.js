...
export default function Edit( { isSelected } ) {
	...
	if ( ! isSelected ) {
		...
		return ( <div { ...blockProps }> { meta[ invp_blocks.meta_prefix + 'stock_number' ] } </div> );
	}
	...
	return (
		<div { ...blockProps }>
			<TextControl
				label    = { 'Stock Number' }
				value    = { meta[ invp_blocks.meta_prefix + 'stock_number' ] }
				onChange = {(newValue) => setMeta( { ...meta, [invp_blocks.meta_prefix + 'stock_number']: newValue } )}
			/>
		</div>
	);
}
